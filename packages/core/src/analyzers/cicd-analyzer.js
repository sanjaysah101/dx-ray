...
class CICDAnalyzer extends BaseAnalyzer {
    ...
    try {
      ...
      if (!pipeline) {
        this.addFinding({
          ...
          description: 'No CI/CD configuration found. Continuous integration is essential for catching bugs early and maintaining code quality.',
        });
        this.addSuggestion({
          title: 'Set Up CI/CD Pipeline',
          description: 'Add a GitHub Actions workflow (.github/workflows/ci.yml) to run tests and linting on every push and PR.',
          priority: 'high',
          impact: 'high',
          effort: 'medium',
        });
  ...
  async _detectPipeline() {
    const ciSystems = [
      { name: 'GitHub Actions', patterns: ['.github/workflows/*.yml', '.github/workflows/*.yaml'] },
    ...
    for (const ci of ciSystems) {
      for (const pattern of ci.patterns) {
        const files = await fastGlob(pattern, { cwd: this.targetDir, absolute: true });
        if (files.length > 0) {
          const configs = [];
          for (const file of files) {
            try {
              const content = await fs.promises.readFile(file, 'utf-8');
              configs.push({ file: path.relative(this.targetDir, file), content });
            } catch {
              // Skip unreadable
            }
          }
  ...
  async _analyzePipelineConfig(pipeline) {
    ...
    for (const config of pipeline.configs) {
      ...
      if (/lint|eslint|biome|prettier|rubocop|flake8/.test(content)) hasLinting = true;
      if (/build|compile|webpack|vite|tsc/.test(content)) hasBuild = true;
      if (/deploy|publish|release|aws|gcp|azure|vercel|netlify/.test(content)) hasDeploy = true;
      if (/slack|discord|email|notify|teams/.test(content)) hasNotifications = true;
    }
    ...
    if (!hasTests) {
      this.addFinding({
        ...
        description: 'CI pipeline does not appear to run tests. Tests should be a mandatory step in every pipeline.',
      });
      this.addSuggestion({
        title: 'Add Test Step to CI',
        description: 'Add a test step to your CI pipeline: `npm test` or equivalent.',
        priority: 'high',
        impact: 'high',
        effort: 'low',
      });
    ...
    if (!hasLinting) {
      this.addFinding({
        ...
        description: 'CI pipeline does not run linting. Linting in CI catches code quality issues before merge.',
      });
      this.addSuggestion({
  ...
  async _checkCaching(pipeline) {
    ...
    if (!hasCaching) {
      this.addFinding({
        ...
        description: 'No dependency caching detected in CI. Caching node_modules or package manager cache can reduce build times by 30-60%.',
      });
      this.addSuggestion({
        title: 'Enable CI Caching',
        description: 'Add `actions/cache` for node_modules or use `actions/setup-node` with built-in caching to speed up builds.',
        priority: 'high',
        impact: 'high',
        effort: 'low',
      });
  ...
  async _checkSecuritySteps(pipeline) {
    ...
    for (const config of pipeline.configs) {
      const content = config.content.toLowerCase();
      if (/snyk|dependabot|codeql|trivy|audit|security|sonar|semgrep/.test(content)) {
        hasSecurity = true;
        break;
      }
    }
    ...
    if (!hasSecurity) {
      this.addFinding({
        ...
        description: 'No security scanning tools detected in CI pipeline. Consider adding dependency auditing and code scanning.',
      });
      this.addSuggestion({
        title: 'Add Security Scanning',
        description: 'Add `npm audit` or integrate Snyk/CodeQL for automated security scanning in CI.',
        priority: 'high',
        impact: 'high',
        effort: 'low',
      });
  ...
  async _checkParallelization(pipeline) {
    ...
    if (!hasParallel && (this.metrics.pipelineSteps?.totalSteps || 0) > 8) {
      this.addFinding({
        ...
        description: 'Pipeline has many steps but no parallelization. Running jobs in parallel can significantly reduce total build time.',
      });
      this.addSuggestion({
        title: 'Parallelize CI Jobs',
        description: 'Split independent steps (lint, test, build) into parallel jobs using matrix strategy or separate workflow jobs.',
        priority: 'medium',
        impact: 'high',
        effort: 'medium',
      });
